...
extern uint8_t main_ESP32_rxBuffer[ESP32_rxBufferSize];
...
uint8_t esp32_txBuffer[ESP32_rxBufferSize];
...
//	response = HAL_UARTEx_ReceiveToIdle_DMA(huart, ESP32_rxBuffer, ESP32_rxBufferSize);
//	__HAL_DMA_DISABLE_IT(ESP32_hdma_usart_purpose, DMA_IT_HT);
//
//    __HAL_DMA_ENABLE(ESP32_huart_channel->hdmarx);
//    __HAL_UART_ENABLE(ESP32_huart_channel);

	  response = HAL_UART_Receive_DMA(huart, main_ESP32_rxBuffer, ESP32_rxBufferSize);
...
void ESP32_receiveDataProcess(void)
{
	...
	if(ESP32_rxBuffer[1] == MB_READ_CMD)
		size = 8;

	else if(ESP32_rxBuffer[1] == MB_WRITE_CMD)
		size = ESP32_rxBuffer[6] + 9;
	...
	if((crc == crcRx)&&(crcRx != 0))
	{
		...
		if(ESP32_rxBuffer[1] == MB_READ_CMD)
		{
			...
			//SEGGER_RTT_printf(0,"txBuffer : %x %x %x %x %x %x \r\n", txBuffer[0], txBuffer[1], txBuffer[2], txBuffer[3], txBuffer[5], txBuffer[6]);
		...
		else if(ESP32_rxBuffer[1] == MB_WRITE_CMD)
		{
			...
			for(int i=0;i<amount;i++)
				ESP32_writeData[i] = combineBytes(ESP32_rxBuffer[7+(i*2)], ESP32_rxBuffer[8+(i*2)]);



			esp32_txBuffer[0] = 0x01;
			esp32_txBuffer[1] = 0x16;
			esp32_txBuffer[2] = ESP32_rxBuffer[2];
			esp32_txBuffer[3] = ESP32_rxBuffer[3];
			esp32_txBuffer[4] = ESP32_rxBuffer[4];
			esp32_txBuffer[5] = ESP32_rxBuffer[5];
		...
		SEGGER_RTT_printf(0,"ESP32 CRC ERROR ! \r\n");
